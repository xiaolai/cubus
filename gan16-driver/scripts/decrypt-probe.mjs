...
const BASE_KEY = [0x01,0x02,0x42,0x28,0x31,0x91,0x16,0x07,0x20,0x05,0x18,0x54,0x42,0x11,0x12,0x53];
const BASE_IV  = [0x11,0x03,0x32,0x28,0x21,0x01,0x76,0x27,0x20,0x95,0x78,0x14,0x32,0x12,0x02,0x43];

function saltedKeyIv(macStr) {
  // salt = MAC bytes in reverse order (upstream gan-smart-cube.ts)
  const salt = macStr.split(/[:\-\s]+/).map(h => parseInt(h, 16)).reverse();
...
if (!file || !mac) { console.error('usage: decrypt-probe.mjs <capture.kv> <MAC>'); process.exit(1); }
...
for (const line of lines) {
  ...
  if (shown < 24) { console.log(`evt=0x${evt.toString(16).padStart(2,'0')} len=${dec[1]}  ${dec.toString('hex')}`); shown++; }
}
