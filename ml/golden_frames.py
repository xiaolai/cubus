        ...
        if self.name == "native":
            ...
            try:
                try:
                    ...
                    r = subprocess.run([str(self.probe), str(self.models / "cube-yolo.mlpackage"), str(png), str(out_bin), self.compute_units], capture_output=True, text=True, timeout=120)
...
def check(args) -> int:
    ...
    if manifest_path.is_file():
        entry = json.loads(manifest_path.read_text()).get("artefacts", {}).get("cube-yolo.mlpackage", {})
