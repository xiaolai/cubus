...
class Leg:
    ...
    def __init__(self, name: str, models: Path, compute_units: str, probe: Path):
        ...
        if name in ("onnx", "onnx-int8"):
            ...
            path = models / ("cubedet.onnx" if name == "onnx" else "cubedet.int8.onnx")
            if not path.is_file():
            ...
            if not (models / "cubedet.mlpackage").exists():
                raise FileNotFoundError(f"{models / 'cubedet.mlpackage'} — run export.py")
        else:
...
def runnable_legs(models: Path, probe: Path) -> list[str]:
    ...
    if platform.system() == "Darwin":
        legs.append("coreml")
    if (models / "cubedet.tflite").is_file():
        legs.append("tflite")
    if platform.system() == "Darwin" and probe.is_file() and (models / "cubedet.mlpackage").exists():
        legs.append("native")
    return legs
