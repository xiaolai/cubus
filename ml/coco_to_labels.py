"""Convert BlenderProc COCO annotations → detector detection labels. Pure stdlib, testable.

BlenderProc writes `coco_annotations.json` (bbox in [x, y, w, h] pixels, per-instance, with
occlusion already accounted for — an occluded sticker gets a clipped/absent box). detector wants
...
# COCO category_id → detector class index. The generator stores colours 1-indexed (white=1..blue=6)
# because BlenderProc reserves category_id 0 for background and drops it; the body is 7. Shift
# 1..6 back to 0..5 here; 7 (body) has no entry, so coco_to_label_lines skips it.
