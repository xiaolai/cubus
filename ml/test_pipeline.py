...
def test_coco_to_labels() -> None:
    ...
    lines = coco_to_label_lines(anns, img_w=100, img_h=200, catid_to_class=DEFAULT_MAP)
    ...
    assert coco_to_label_lines([{"category_id": 0, "bbox": [0, 0, 50, 50]}], 100, 100, DEFAULT_MAP) == []
    assert coco_to_label_lines([{"category_id": 7, "bbox": [0, 0, 50, 50]}], 100, 100, DEFAULT_MAP) == []
    # unknown category is skipped
    assert coco_to_label_lines([{"category_id": 99, "bbox": [0, 0, 50, 50]}], 100, 100, DEFAULT_MAP) == []
    print("PASS coco_to_labels: 1-indexed shift, normalization, area filter, background/body/unknown skip")
...
def test_shipped_int8_is_derived_from_the_shipped_fp32() -> None:
    """`ml/models/cubedet.int8.onnx` must be `quantize_dynamic` of the fp32 file beside it.
    ...
    print("PASS models: cubedet.int8.onnx is quantize_dynamic(cubedet.onnx), and the manifest says so")
...
def test_the_tflite_leg_converts_a_copy_and_takes_the_one_float32_graph() -> None:
    ...
    asked for, and exactly one `cubedet_float32.tflite` is accepted.
    """
    import sys
    import tempfile
    import types
    ...
    calls: list[dict] = []
    written: list[str] = ["cubedet_float32.tflite"]
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            written[:] = ["cubedet_float16.tflite", "cubedet_integer_quant.tflite"]
            calls.clear()
            try:
                export.export_tflite(fp32, root / "work-other", out)
            except SystemExit as e:
                assert "expected one cubedet_float32.tflite" in str(e), e
                assert "cubedet_float16.tflite" in str(e), "the refusal does not say what was there"
            else:
...
def test_the_metrics_table_scores_the_set_its_dataset_file_names() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            model = root / "cubedet.onnx"
            model.write_bytes(b"not really a model")
            row = mt.validate(model, yaml_path)
            assert scored == {"model": "cubedet.onnx", "files": ["a.jpg", "b.png"], "labels": "labels"}, scored
...
def test_a_coreml_package_is_identified_by_its_model_not_its_random_ids() -> None:
    ...
    shipped = HERE / "models" / "cubedet.mlpackage"
    ...
    recorded = json.loads((HERE / "models" / "MANIFEST.json").read_text())["artefacts"]["cubedet.mlpackage"]["sha256"]
