"""Which cube each labelled sticker is on: the one fact a detector label file has no column for.

A detector row is `class cx cy w h`. Nothing in it says which cube the sticker belongs to, so a frame
...
identity travels beside the labels, in a file of its own:
    ...
    <root>/labels/<split>/<stem>.txt   the detector rows, untouched (every trainer reads only these)
...
label file with no cube file at all. Producers that know write it: merge_parts.py and coco_to_labels.py
...
def write_cubes(path: Path, ids: list[int]) -> None:
    """One integer per label row. An empty file for a frame with no rows, as detector does for labels."""
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Write a detector tree's cube files from the photographs' recorded check.")
