...
| `render.sh` | render N scenes → COCO → detector → split | the training host |
| `train.sh` | v3n fine-tune in NGC arm64 container → ONNX | the training host |
...
- Copy `best.onnx` → `app/renderer/vendor/cubedet.onnx`.
