...
Train a v3n model that detects the 9 sticker colors of a cube face, robustly, under any
...
```
Batch can be large (GB10 shares ~113 GB unified memory). The starting `base11n.pt` must be present
...
# 2. Train (near GPU box). Detached; refuses unless the pinned base11n.pt is present.
...
ml/venv/bin/python ml/color_eval.py --model ml/models/cubedet.onnx --images ml/out/heldout/images --labels ml/out/heldout/labels
```
...
- [x] **Training image pinned** — `Dockerfile.train` pins the NGC base by digest and the
      detlib/onnxruntime versions, and asserts the pins took; `train.sh` pins `base11n.pt` by md5.
