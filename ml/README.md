...
**legacy**: the Detlib v3n pipeline that produced v3, the model it replaced, is kept so v3's
...
| `train.sh` | **legacy:** v3n fine-tune in the pinned `cube-train:1` container → `best.pt` | the near GPU box |
...
`cube-train:1` (it refuses unless the pinned `base11n.pt` is present and matches its md5), then
...
- [x] **Training environment clean** — `run-cubedet.sh` uses the NGC image with no detlib in
      ...
      `Dockerfile.train` pins v3's detlib/onnxruntime and `train.sh` pins `base11n.pt` by md5.)
