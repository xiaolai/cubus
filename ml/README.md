...
copyleft code anywhere on the path. Everything here describes that pipeline. The Ultralytics
...
- **Train on the near GPU box** (GB10 CUDA). `run-cubedet.sh` runs the trainer in the clean NGC
  PyTorch image, which has no ultralytics in it. The far box is identical silicon but 0.1 MB/s away:
...
| `coco_to_yolo.py` | BlenderProc COCO → YOLO labels, and each row's cube (pure) | ✅ `test_pipeline.py` |
...
| `merge_parts.py` | merge parallel worker parts → one YOLO set, with its cube files (pure) | ✅ `test_pipeline.py` |
| `split_dataset.py` | train/val split into YOLO layout; refuses a label without its cube file unless `--no-cubes` | ✅ `test_pipeline.py` |
...
### v3
The Ultralytics pipeline that produced v3 — `train.sh`, `train_mps.sh`, `Dockerfile.train`,
`requirements-train.txt`, and `export.py`'s Ultralytics path — was removed on 2026-09-18; git history
holds it. The numbers in `MODEL_CARD.md` and `OOD_EVAL.md` dated before 2026-09-17 are v3's, and the
mAP rows among them came from Ultralytics' validator (`yolo val`), which `metrics_table.py` no longer
