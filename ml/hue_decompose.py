...
def cube_of(bbox, bodies) -> int | None:
    ...
    -1 when the loader has no body rows at all (the YOLO path): the frame is taken as one cube.
    """
    if not bodies:
        return -1
...
def load_yolo(root, budget, rng):
    ...
    YOLO labels carry no cube body, so every sticker comes back as cube -1 and the frame is treated
    ...
    for lp in labels:
        ...
        # One cube shows at most three faces, 27 stickers. More than that in a YOLO file is several
