...
| cube-yolo.int8.onnx   | NOT SHIPPED (onnxruntime-web)    | the above, `quantize_dynamic` (QInt8) — written only if it still reads (see `int8_reads_a_face`) |
| cube-yolo.mlpackage   | Apple (CoreML, macOS + iOS)      | ML program; fp32 for cubedet (measured faster AND exact), fp16 for the Ultralytics path |
| cube-yolo.tflite      | Android APK, gated OFF (LiteRT)  | fp32 for cubedet; dynamic-range int8 for the Ultralytics path |
...
ARTEFACT_LABELS: dict[str, dict[str, str]] = {
    ...
    # The precision here is the CUBEDET path's, because that is what ships. The Ultralytics path still
...
def export_onnx_cubedet(pt: Path, work: Path, out: Path) -> tuple[Path, Path | None, str]:
    """The same ONNX artefacts, from a `cubedet` checkpoint and with no Ultralytics anywhere.
...
def int8_reads_a_face(fp32: Path, int8: Path) -> tuple[bool, str]:
    ...
    leaving the head's convolutions in fp32 all give the same collapse). v3's Ultralytics graph
...
def export_coreml_cubedet(pt: Path, work: Path, out: Path) -> Path:
    """CoreML from the same checkpoint, at fp32 — unlike the Ultralytics path above, which is fp16.
...
def main(argv: list[str] | None = None) -> None:
    ...
    # shipping, so a bare `export.py` rebuilt the retired Ultralytics model over the committed
    ...
    if "coreml" not in args.skip:
        ...
        if not args.cubedet:  # the Ultralytics path converts at fp16; see export_coreml_cubedet for why cubedet does not
    ...
    if "tflite" not in args.skip:
        ...
        if not args.cubedet:  # the Ultralytics graph loses nothing to weight-only int8, so it keeps it
