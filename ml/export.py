...
| cube-yolo.mlpackage   | Apple (CoreML, macOS + iOS)      | ML program, fp32 (measured faster AND exact)       |
| cube-yolo.tflite      | Android APK, gated OFF (LiteRT)  | fp32                                              |
...
Every artefact comes from a `cubedet` checkpoint (ml/cubedet), with no Ultralytics code on any path.
The Ultralytics pipeline that produced v3 — its training scripts, its image, and this file's export
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Record that the model has no AGPL lineage, and carry the training environment the
