...
| cubedet.int8.onnx   | web (onnxruntime-web, all desktop)| the above, onnxruntime `quantize_dynamic` (QInt8) |
| cubedet.mlpackage   | Apple (CoreML, macOS + iOS)      | ML program, fp16 compute, fp32 tensor IN, fp16 OUT |
| cubedet.tflite      | Android (LiteRT/TFLite, XNNPACK) | dynamic-range int8 (int8 weights, fp32 activations), fp32 I/O — full-int8 collapses the head |
