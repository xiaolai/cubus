...
| cubedet.tflite      | NOT SHIPPED (LiteRT/TFLite)      | dynamic-range int8 (int8 weights, fp32 activations), fp32 I/O — full-int8 collapses the head |
...
apps/web/vendor/cubedet.onnx is byte-identical to the fp32 graph, and `tflite` said "Android"
...
def write_calibration_yaml(images: Path, work: Path) -> Path:
    """A minimal detector dataset yaml over a directory of images: calibration needs pixels, not labels."""
    import yaml
...
def main() -> None:
    ...
    manifest["tools"].update({"detlib": detlib.__version__, "torch": torch.__version__, "onnx": onnx.__version__, "onnxruntime": onnxruntime.__version__})
    ...
    if "onnx" not in args.skip:
        ...
        manifest["artefacts"][int8.name] = {
            ...
            "quantisation_note": "quantises ACTIVATIONS as well as weights, and that is what costs the reads: 4 of 20 golden fixtures come back with a different face, two of them frames the fp32 graph correctly refuses. Contrast cubedet.tflite, which takes the same size reduction weight-only and diverges on none. Do not ship this without re-exporting weight-only.",
        }
    ...
    # The CoreML and TFLite steps re-run detlib, which rewrites cubedet.onnx underneath an
