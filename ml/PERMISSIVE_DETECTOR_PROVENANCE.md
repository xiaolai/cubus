...
those models. That is why cubus is AGPL-3.0 rather than permissive, and it is why a closed or paid
product built on this repository would additionally need an Ultralytics Enterprise Licence.
...
1. the **trainer** — the `ultralytics` package itself; and
...
distinction from the Ultralytics situation is not that one is copyleft-free by luck. It is that
Ultralytics *actively asserts* its licence reaches models trained with its software, and neither
...
implementation's source was opened while writing this code — not Ultralytics', and not a permissive
one either, because a clean-room claim that quietly leans on a reimplementation of an AGPL codebase
...
`cubedet/train.py` refuses to start if `ultralytics` is importable in the training environment, and
...
refusal requires `--allow-agpl-in-env`, which warns on stderr and records
...
mentions Ultralytics once, in a comment, and its pinned dependencies are numpy, pillow,
...
- [ ] Licence changed from AGPL-3.0 — **the owner's call, and not implied by this work**
