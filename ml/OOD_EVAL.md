...
| `metrics_table.py` | Emits the mAP rows of `MODEL_CARD.md` and this file from the repository's own evaluator (`cubedet.val` via `compare_detectors.score`, imgsz 640, CPU; Detlib's `yolo val` until 2026-09-18), naming each artefact by sha256 — and writes the `--metrics` JSON `ood_report.py` reads, so no number in the report is a literal. |
