...
| `metrics_table.py` | Emits the mAP rows of `MODEL_CARD.md` and this file from `yolo val` (detlib' validator, imgsz 640, CPU), naming each artefact by sha256 — and writes the `--metrics` JSON `ood_report.py` reads, so no number in the report is a literal. |
