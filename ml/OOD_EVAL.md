# Honest evaluation: measuring cubedet out-of-distribution
...
python ood_eval.py    --model ../app/renderer/vendor/cubedet.onnx --images /tmp/ood --out /tmp/ood_out
python ood_gallery.py --out /tmp/ood_out                                   # local review grid
python ood_report.py  --out /tmp/ood_out --dest ../dev-docs/artifacts/cubedet-ood-report.html \
...
detector val model=out/cube_best.pt data=out/heldout/data.yaml imgsz=640 device=mps
```
...
6 centres (CIEDE2000) — the classical path's method. Built as a hybrid (detector localizes, `assemble`
...
actual detector. A 30k-image detector is a far stronger absolute classifier than the ELM/SVM the
