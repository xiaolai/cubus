...
| `metrics_table.py` | Emits the mAP rows of `MODEL_CARD.md` and this file from the repository's own evaluator (`cubedet.val` via `compare_detectors.score`, imgsz 640, CPU; Ultralytics' `yolo val` until 2026-09-18), naming each artefact by sha256 — and writes the `--metrics` JSON `ood_report.py` reads, so no number in the report is a literal. |
...
| `prep_heldout.py` | Turns a downloaded Roboflow cube dataset into a YOLO-layout labelled set in our 6-colour scheme (remap by name, drop non-colour, seg→box). Reports the mapping so an incompatible set is caught. |
...
repository's own evaluator (`metrics_table.py`, which no longer runs `yolo val` — compare these rows
