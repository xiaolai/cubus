...
collides, and converts the boxes to YOLO labels on the way (reusing coco_to_yolo). Beside the labels,
...
from coco_to_yolo import DEFAULT_MAP, coco_to_yolo_rows
...
def merge(out: str, parts: list[str]) -> tuple[int, int]:
    ...
    for pi, part in enumerate(sorted(parts)):
        ...
        for img_id, im in images.items():
            ...
            lines, cubes = coco_to_yolo_rows(per_image.get(img_id, []), im["width"], im["height"], DEFAULT_MAP)
