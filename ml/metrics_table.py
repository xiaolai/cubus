#!/usr/bin/env python
"""The mAP tables in MODEL_CARD.md and OOD_EVAL.md, emitted from `detector val` rather than typed in.
...
Every row is one (model, dataset) run of detlib' validator at imgsz 640 on the CPU — the same
...
DEFAULT_MODELS = [HERE / "out" / "cube_v3_best.pt", HERE / "models" / "cubedet.onnx", HERE / "models" / "cubedet.int8.onnx"]
...
def validate(model_path: Path, data: Path, workdir: Path) -> dict:
    ...
    r = detector(str(model_path)).val(data=str(data), imgsz=640, device="cpu", plots=False, verbose=False, project=str(workdir), name=model_path.stem, exist_ok=True)
...
def main() -> int:
    ...
    ap.add_argument("--workdir", type=Path, default=Path("/tmp/cube-metrics"), help="where detlib writes its run dirs")
    args = ap.parse_args()
    os.environ.setdefault("DET_OFFLINE", "1")  # never fetch fonts, updates or datasets during a measurement
    if args.dataset:
        DATASETS.clear()
    ...
    print(f"\ndetlib {detlib.__version__}, imgsz 640, CPU; {sizes}; heldout has {removed} near-duplicates removed by dedup_heldout.py\n")
    ...
    if args.json:
        ...
        doc = {
            ...
            "tool": {"detlib": detlib.__version__, "imgsz": 640, "device": "cpu"},
