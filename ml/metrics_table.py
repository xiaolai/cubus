...
Ultralytics' validator (`yolo val`); that tool is gone from the repository with the rest of the
Ultralytics pipeline, and a row it produced is labelled with its version where a document still
...
DEFAULT_MODELS = [p for p in (HERE / "models" / "cube-yolo.onnx", HERE / "models" / "cube-yolo.int8.onnx") if p.is_file()]
...
def evaluator_digest() -> str:
    ...
    The Ultralytics rows carried a tool VERSION; this one is the repository's own code, so a change to
...
def main() -> int:
    ...
    if not args.models:
        sys.exit("no model to score: ml/models/cube-yolo.onnx is missing")
