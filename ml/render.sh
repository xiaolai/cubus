...
# SCENES * POSES. After all workers finish we merge the parts → detector labels → train/val split.
#
...
render_worker() {
  ...
  for ((s = w; s < SCENES; s += WORKERS)); do
    "$BLENDERPROC" run "$HERE/generate_cube_dataset.py" -- \
      --output_dir "$part" --hdri_dir "$HDRI_DIR" --num_poses "$POSES" --res "$RES" --seed "$s"
  done
  echo "  worker $w done"
}
...
echo "Merging $WORKERS parts -> detector labels…"
