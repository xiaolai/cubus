...
// on the DOM-lowercased spelling, so both are observed and normalized in _set().
  ...
  static _directedCurve(points, { radius, segments, headRadius, headLength, headSides, lift, material }) {
    ...
    const head = new THREE.Mesh(new THREE.ConeGeometry(headRadius, headLength, headSides), material);
  ...
  _placeArrow() {
    ...
    const { tube, head, end } = CubusCube._directedCurve(points, {
      radius: 0.055, segments: 48, headRadius: 0.15, headLength: 0.34, headSides: 20, lift: 0.12,
  ...
  _placeTrails() {
    ...
    for (const [n, { token, index }] of named.entries()) {
      ...
      const { tube, head } = CubusCube._directedCurve(lifted, {
        radius: 0.045, segments: lifted.length * 3, headRadius: 0.12, headLength: 0.28, headSides: 16, lift: 0.1,
        material,
      });
  ...
  playTo(k) {
    const sol = this._sol;
    this._settleGroup();
    // The settle reports every token it lands, and a host may dispose this element or write a new `alg`
    // from inside one — the same question `stepStop` asks, for the same reason.
    if (!this.stickers || this._sol !== sol) return;
