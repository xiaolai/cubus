...
refused, and a link that dropped or respawned — and compares that count across the pre-read.
Before the write it refuses and transmits nothing. After it, the packet has gone, so it reports
the anchor as **uncertain** and says to re-scan; that is the honest answer, not a verification.
`force` does not waive this check: it vouches for the cube the caller is looking at, and a cube
that has turned since is not that cube.
...
`tests/anchor-race.test.ts` pins both interleavings.
