...
async function main() {
  ...
  console.log(
    `FOUND ${cube.name}  mac=${mac}  name-suffix ${macMatchesName(mac, cube.name) ? 'OK' : 'MISMATCH'}`,
  );

  const gan = new GanCube({ mac, transport: new BlewTransport(cube.id) });
  const moves: string[] = [];
  gan.onMove((m) => {
    moves.push(m.notation);
    console.log(`  MOVE ${m.notation} serial=${m.serial}`);
  });
  ...
  console.log(
    `  HARDWARE name=${hw.hardwareName} hw=${hw.hardwareVersion} sw=${hw.softwareVersion} date=${hw.productDate} gyro=${hw.gyroSupported} extras=${hw.extras.map((e: any) => `0x${e.eventType.toString(16)}`).join(',')}`,
  );
