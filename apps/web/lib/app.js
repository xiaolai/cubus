...
import {
  lessonCues, lessonSections, moveStepIndex, rungSummary,
} from './method-lesson.js';
import {
  acceptOffer, followsUntilOffer, ladderRows, repairProgress,
} from './method-ladder.js';
...
import {
  METHOD_FRAME, METHOD_TO_SCAN, renameAlg, renameSelectors, toMethodFrame,
 ...
 */
function lessonFor(c = state.cube) {
 ...
 * block below, and `provenMinimumLabel` behind `showingProof`. A module elsewhere is a place that
 * scanner does not read, which is the one thing a claim may never have. The session decides what
 * the walk is; this is where the app says what may be claimed about it.
 *
 ...
 */
function sayWalkLength({ root, setStatus, scrambling, route, stageTargetNow, lesson, total, steps, fresh }) {
