...
/** "Where does this piece live?" — a scrambled cube, one slot lit, the piece in it not at home. */
...
/** "Where will this piece be after this turn?" — two of the first four are turns that do not touch it. */
...
export const recognitionScript = (r) => ({
  ...
  steps: [
    { hl: `slot:${r.slot}`, cam: r.cam, say: 'Where does this piece live?' },
    { round: { ask: `pieceIn:${r.slot}`, choose: r.slot.length, reveal: [{ hl: `slot:${r.piece}`, say: 'It lives here.' }] } },
  ],
});
...
export const predictionScript = (r) => ({
  ...
  start: { scramble: r.alg },
  steps: [
    { hl: `piece:${r.piece}`, cam: r.cam, say: 'Where will this piece be?' },
