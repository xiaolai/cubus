...
test('every challenge is a real position with self-consistent proof numbers', async () => {
  // The artifact is generated by `cargo run --release -p optimal-solver --bin gen-library`
  // and COMMITTED, so its absence is a missing shipped file, never a checkout mid-generation.
  // This used to t.skip() on that condition, which meant losing the app's proven data table
  // read as a green suite — the quiet default this repo refuses everywhere else.
  assert.ok(existsSync(data),
    'lib/data/optimal-challenges.json is missing — it is committed, so this is a lost file, ' +
    'not an ungenerated one; restore it or regenerate with the gen-library binary');
