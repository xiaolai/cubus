...
const SHIPPED = new URL('../vendor/cubedet.onnx', import.meta.url);
const REFERENCE = new URL('../../../ml/models/cubedet.onnx', import.meta.url);
const INT8 = new URL('../../../ml/models/cubedet.int8.onnx', import.meta.url);

test('the browser is served the fp32 reference graph, byte for byte', () => {
  assert.ok(existsSync(SHIPPED), 'apps/web/vendor/cubedet.onnx is missing — the scanner has no model');
  if (!existsSync(REFERENCE)) {
    // ml/models is present in a clone, so this is a real absence rather than a gitignored one.
    assert.fail('ml/models/cubedet.onnx is missing — cannot verify which export is shipped');
  }
...
// resolved `cubedet.onnx` and `tauri.windows.conf.json` declared no `bundle.resources` at all,
...
//
// `gen/apple/assets/models/cubedet.mlpackage` is a copy of `ml/models/cubedet.mlpackage` that
...
const IOS_MODEL = '../../desktop/src-tauri/gen/apple/assets/models/cubedet.mlpackage';
const ML_MODEL = '../../../ml/models/cubedet.mlpackage';
...
test('the committed iOS model is the one in ml/models, file for file', () => {
  ...
  assert.ok(existsSync(source), 'ml/models/cubedet.mlpackage is missing');
  ...
  const drifted = MLPACKAGE_FILES.filter((f) => {
    ...
    assert.ok(existsSync(a), `ml/models/cubedet.mlpackage/${f} is missing`);
